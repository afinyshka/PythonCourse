...
def find_entry_del_widget(texter):
    ...
    sc.config(command=output_line_2.yview)
    output_line_2.insert(1.0, output_text)
    clear_1 = tk.Button(win, text = "Clear", command=lambda: widgets_remove(output_line_1, output_line_2, clear_1, sc))
    clear_1.grid(row=6, column=1, stick="we")

def widgets_remove(output_line_1, output_line_2, clear_1, sc):
